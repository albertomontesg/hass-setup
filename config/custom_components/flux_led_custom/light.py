...
from homeassistant.components.light import (
    ...
    SUPPORT_COLOR, SUPPORT_WHITE_VALUE, LightEntity, PLATFORM_SCHEMA)
...
class FluxLight(LightEntity):
    ...
    def get_rgb(self):
      ...
      return tuple((rgb[0], rgb[2], rgb[1]))
      
    def get_rgbw(self):
      rgbw = self._bulb.getRgbw()
      return tuple((rgbw[0], rgbw[2], rgbw[1], rgbw[3]))
      
    def set_rgbw(self, r=None, g=None, b=None, w=None):
      self._bulb.setRgbw(r=r, g=b, b=g, w=w)
    ...
    def _turn_on(self, **kwargs):
        ...
        if self._mode == MODE_WHITE:
            self.set_rgbw(0, 0, 0, w=brightness)

        # handle RGBW mode
        elif self._mode == MODE_RGBW:
            if white is None:
                self.set_rgbw(*color_util.color_hsv_to_RGB(*self._color))
            else:
                self.set_rgbw(w=white)
        # handle RGB mode
        else:
            self.set_rgbw(*color_util.color_hsv_to_RGB(*self._color))
        return
